...
@onready var health_label:Node= $Healthbar/HealthThing/HealthLabel
@onready var armor: TextureProgressBar = $Armor
@onready var hurt_anim: AnimationPlayer = $Healthbar/HealthThing/AnimationPlayer
...
func _ready() -> void:
	...
	original_position = health_label.position
...
func health_changed(health):
	...
	add_quake(0.75)
	hurt_anim.play("RESET")
	hurt_anim.play("HurtFlash")
...
func _process(delta):
	...
	health_label.position = original_position + quake_offset
